...
def should_use_background_rgb(
    dataset,
    scene: Scene,
    has_train_mask: bool,
    iteration: int,
    stage_a_geo_from_iter: int,
    stage_a_end_iter: int,
) -> bool:
    if not dataset.train_with_background_rgb or not scene.should_train_with_bg:
        return False
    if has_train_mask:
        return False
    # Disable background splats during Stage-A geometry-focused window.
    if stage_a_geo_from_iter <= iteration <= stage_a_end_iter:
        return False
    return True
...
def training(
    ...
    low_resolution = get_low_resolution(dataset)
    stage_a_end_iter = max(1, min(int(stage_schedule["stage_a_end_iter"]), int(opt.iterations)))
    stage_a_geo_from_iter = max(1, min(int(stage_schedule["stage_a_geo_from_iter"]), stage_a_end_iter))
    stage_b_start_iter = stage_a_end_iter + 1
    stage_b_final_prune_from_iter = max(stage_b_start_iter, int(stage_schedule["stage_b_final_prune_from_iter"]))
    stage_b_final_prune_interval = max(0, int(stage_schedule["stage_b_final_prune_interval"]))
    ...
    print(f"[Pipeline] Stage A: [1, {stage_a_end_iter}] (geo from {stage_a_geo_from_iter})")
    print(f"[Pipeline] Stage B: [{stage_b_start_iter}, {opt.iterations}]")
    ...
    print(
        f"[Pipeline] Stage B final prune: from {stage_b_final_prune_from_iter}, "
        f"interval={stage_b_final_prune_interval}"
    )
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        in_stage_a = iteration <= stage_a_end_iter
        in_stage_b = iteration > stage_a_end_iter
        if reflective_case:
            lambda_multi_view_ncc_cur = 0.1 if in_stage_a else 0.0
            if iteration < 3000:
                lambda_normal_prior_cur = 0.0
            elif 3000 <= iteration < 7000:
                lambda_normal_prior_cur = 0.2 * (iteration - 3000) / 4000.0
            elif 7000 <= iteration <= stage_a_end_iter:
                ramp_denom = max(stage_a_end_iter - 7000, 1)
                lambda_normal_prior_cur = 0.2 + 0.1 * (iteration - 7000) / ramp_denom
            else:
                lambda_normal_prior_cur = 0.0
        else:
            lambda_multi_view_ncc_cur = 0.6 if in_stage_a else 0.0
            lambda_normal_prior_cur = 0.0

        reg_kick_on = in_stage_a and (iteration >= stage_a_geo_from_iter)
        normal_prior_kick_on = in_stage_a and reflective_case and lambda_normal_prior_cur > 0
        ...
        bg_model = scene.bg_gaussians if should_use_background_rgb(
            dataset,
            scene,
            has_loaded_mask,
            iteration,
            stage_a_geo_from_iter,
            stage_a_end_iter,
        ) else None
        render_pkg = render(
            viewpoint_cam,
        ...
        with torch.no_grad():
            ...
            # Stage-B final pruning for render-oriented point count control.
            if (
                opt.vcp_enable
                and in_stage_b
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stage_a_end_iter", type=int, default=12000)
    parser.add_argument("--stage_a_geo_from_iter", type=int, default=7000)
    parser.add_argument("--stage_b_final_prune_from_iter", type=int, default=15000)
    parser.add_argument("--stage_b_final_prune_interval", type=int, default=3000)
    args = parser.parse_args(sys.argv[1:])

    args.stage_a_end_iter = max(1, min(args.stage_a_end_iter, args.iterations))
    args.stage_a_geo_from_iter = max(1, min(args.stage_a_geo_from_iter, args.stage_a_end_iter))
    args.test_iterations = sorted(set(args.test_iterations + [args.stage_a_end_iter, args.iterations]))
    args.save_iterations = sorted(set(args.save_iterations + [args.stage_a_end_iter, args.iterations]))
    args.checkpoint_iterations = sorted(set(args.checkpoint_iterations + [args.stage_a_end_iter, args.iterations]))
    ...
    stage_schedule = {
        "stage_a_end_iter": args.stage_a_end_iter,
        "stage_a_geo_from_iter": args.stage_a_geo_from_iter,
        "stage_b_final_prune_from_iter": args.stage_b_final_prune_from_iter,
        "stage_b_final_prune_interval": args.stage_b_final_prune_interval,
    }
